...
class MetricsCalculator:
    ...
    def calculate_regression_metrics(
        y_true: np.ndarray, y_pred: np.ndarray, n_features: int = None
    ) -> Dict[str, float]:
        """
    ...
    def calculate_classification_metrics(
        ...
        return {
            ...
            "precision_weighted": MetricsCalculator.precision(
...
class ModelEvaluator:
    ...
    def evaluate_regression_models(
        self, models_dict: Dict[str, Any], X: pd.DataFrame, y: pd.Series
    ) -> pd.DataFrame:
        """
    ...
    def generate_comparison_report(
        ...
        if regression_results is not None and len(regression_results) > 0:
            ...
            report_lines.append(
                f"Best Model: {best['model']} (RMSE: {best['rmse']:.2f})"
            )
            report_lines.append("")
        ...
        if classification_results is not None and len(classification_results) > 0:
            ...
            report_lines.append(
                f"Best Model: {best['model']} (F1: {best['f1_weighted']:.3f})"
            )
            report_lines.append("")
    ...
    def save_evaluation(self, results: Dict[str, Any], filename: str = None) -> Path:
        """
...
if __name__ == "__main__":
    ...
    class_metrics = MetricsCalculator.calculate_classification_metrics(

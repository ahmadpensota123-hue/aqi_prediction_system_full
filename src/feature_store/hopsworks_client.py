...
class HopsworksClient:
    ...
    def __init__(self, api_key: str = None, project_name: str = None):
        ...
        self.project_name = (
            project_name or settings.feature_store.hopsworks_project_name
        )
        ...
        if not self.api_key:
            logger.warning(
                "Hopsworks API key not set. " "Get one at: https://app.hopsworks.ai/"
            )
    ...
    def create_feature_group(
        ...
        if primary_key is None:
            primary_key = (
                ["city", "timestamp"] if "timestamp" in df.columns else ["city"]
            )
        ...
        try:
            ...
            logger.info(
                f"Created/updated feature group: {name} v{version} with {len(df)} rows"
            )
    ...
    @log_execution_time
    def get_feature_group(self, name: str, version: int = 1) -> Any:
        """
    ...
    def create_feature_view(
        ...
        try:
            ...
            fv = self._fs.get_or_create_feature_view(
                name=name, version=version, query=query
            )
    ...
    def get_training_data(
        ...
        try:
            ...
            if start_time and end_time:
                X, y = fv.training_data(start_time=start_time, end_time=end_time)
            else:
...
class FeatureStoreManager:
    ...
    @log_execution_time
    def ingest_features(
        self, df: pd.DataFrame, feature_group: str = "aqi_features"
    ) -> bool:
        """
    ...
    @log_execution_time
    def get_training_data(
        self, feature_group: str = "aqi_features", days_back: int = 30
    ) -> pd.DataFrame:
        """
        ...
        if self._connected:
            try:
                ...
                X, _ = self.client.get_training_data(
                    feature_view_name=fv_name, start_time=start_time, end_time=end_time
                )

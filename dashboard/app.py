...
def main():
    ...
    with st.sidebar:
        ...
        # City selection
        cities = ["Beijing", "London", "New York", "Delhi", "Tokyo", "Sydney", "Islamabad", "Lahore", "Karachi"]
    ...
    st.markdown(f"**City:** {city} | **Last Updated:** {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    
    # Fetch data
    current_aqi = fetch_current_aqi(city.lower())
    forecast_df = fetch_forecast(city.lower(), forecast_days, current_aqi_val=current_aqi.get("aqi"))
    ...
    if current_aqi["aqi"] > 150:
        st.warning(f"⚠️ **Health Alert:** AQI is {current_aqi['category']}. {current_aqi['health_message']}")
    
    if current_aqi["aqi"] > 300:
        st.error(f"🚨 **HAZARDOUS CONDITIONS:** Stay indoors and use air purifiers. Avoid outdoor activities.")
    ...
    if not precautions:
        ...
        precautions = cat_info.get("precautions", ["Wear a mask.", "Reduce outdoor activity."])
    ...
    for category, info in AQI_CATEGORIES.items():
        fig.add_hrect(
            y0=info["min"], y1=info["max"],
            fillcolor=info["color"], opacity=0.2,
            line_width=0
        )
    ...
    with col2:
        ...
        category_df = pd.DataFrame([
            {"Category": name, "Range": f"{info['min']}-{info['max']}", "Color": info["color"]}

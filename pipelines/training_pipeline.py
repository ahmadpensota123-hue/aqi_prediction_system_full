...
def load_training_data(use_synthetic: bool = False, n_samples: int = 1000) -> Any:
    """
...
@task(name="Prepare Features", description="Prepare features for model training")
def prepare_features(
    df: Any, target_column: str = "aqi", forecast_horizon: int = 1
) -> Dict[str, Any]:
    """
    ...
    y_regression = df["aqi"]
    y_classification = (
        df["aqi_category_name"]
        if "aqi_category_name" in df.columns
        else df["aqi_category"]
    )
...
def train_regression(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    results = models.train(
        data["X_train"], data["y_regression_train"], validation_split=0.2
    )

    # Evaluate on test set
    test_metrics = models.evaluate(data["X_test"], data["y_regression_test"])
...
def train_classification(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    results = models.train(
        data["X_train"], data["y_classification_train"], validation_split=0.2
    )

    test_metrics = models.evaluate(data["X_test"], data["y_classification_test"])
...
@task(name="Save Models", description="Save trained models to Model Registry")
def save_models(
    regression_results: Dict, classification_results: Dict, timeseries_results: Dict
) -> Dict[str, str]:
    """
...
@task(name="Generate Report", description="Generate training report")
def generate_report(
    regression_results: Dict, classification_results: Dict, timeseries_results: Dict
) -> str:
    """
    ...
    if "test_metrics" in regression_results:
        for name, metrics in regression_results["test_metrics"].items():
            lines.append(
                f"  {name}: RMSE={metrics['rmse']:.2f}, R²={metrics['r2']:.3f}"
            )
    ...
    if "test_metrics" in classification_results:
        for name, metrics in classification_results["test_metrics"].items():
            lines.append(
                f"  {name}: Accuracy={metrics['accuracy']:.3f}, F1={metrics['f1_weighted']:.3f}"
            )
    ...
    report_path = (
        report_dir / f"training_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
    )
...
def training_pipeline(
    use_synthetic_data: bool = True, n_samples: int = 1000, forecast_horizon: int = 1
) -> Dict[str, Any]:
    """
    ...
    results = {"status": "success", "execution_time": None, "errors": []}
    ...
    try:
        ...
        saved_paths = save_models(
            regression_results, classification_results, timeseries_results
        )
...
if __name__ == "__main__":
    ...
    results = training_pipeline(use_synthetic_data=True, n_samples=500)
